...
@Component({
  selector: 'dln-country-orders',
  styleUrls: ['./country-orders.component.scss'],
  template: `
    <nb-card [size]="breakpoint.width >= breakpoints.md ? 'medium' : 'giant'">
      <nb-card-header>Country Orders Statistics</nb-card-header>
      <nb-card-body>
        <dln-country-orders-map (select)="selectCountryById($event)"
                                countryId="USA">
        </dln-country-orders-map>
        <dln-country-orders-chart [countryName]="countryName"
                                  [data]="countryData"
                                  [labels]="countriesCategories"
                                  maxValue="20">
        </dln-country-orders-chart>
      </nb-card-body>
    </nb-card>
  `,
})
